...
| `backtest.py` | `Backtest` class: rolling out-of-sample backtest of all methods + summary statistics |
| `example_usage.py` | End-to-end script: download data, run all methods in-sample, print results |
| `example_backtest.py` | End-to-end script: download data, run the rolling backtest, print the OOS summary |
...
Naive diversification, `wᵢ = 1/N`. An estimation-free benchmark that is hard to beat out-of-sample (DeMiguel, Garlappi & Uppal 2009).

## Rolling out-of-sample backtest

`backtest.py` provides a `Backtest` class for evaluating the methods out-of-sample. For each window it estimates weights on a fixed-length look-back window, holds them over the next `oos_window` periods, and records the realized returns. The look-back window advances by `step` periods each iteration.

- **Non-overlapping** (`step == oos_window`, the default): every held-out period is realized exactly once and the windows join into one continuous return series.
- **Overlapping** (`step < oos_window`): the summary works on compounded holding-period returns.
- **Per-window universe**: only assets with complete data over the look-back window are used, recomputed every window so an asset re-enters as soon as it is fully observed again and the windows stay independent. Out-of-sample gaps are zero-filled and the weights renormalized over the assets present each period.

`summary()` yields, per method, annualized return / volatility / Sharpe / Sortino, maximum drawdown, cumulative return, average turnover, effective N (`1 / Σwᵢ²`), and the realized-vs-predicted volatility ratio.
...
In-sample comparison of all methods:
